...
const calculateInput = (roll) => {
  ...
  for (let i = 0; i < roll.times; i++) {
    let rolled = Math.floor(Math.random() * roll.dice) + 1
    sum += rolled
    rolls.push(rolled)
  }
  sum += roll.modifier
...
const errorMessage = 'You must specify dice and modifiers in following format:\n' +
  '/roll <NDM>\n' +
  'N = Number of dices\n' +
...
const getInlineDiceList = (messageUsername, messageQuery) => {
  ...
  if (messageQuery) {
    results = results.filter((elem) => elem.title.indexOf(
      messageQuery.toUpperCase()) > -1
    )
  }
